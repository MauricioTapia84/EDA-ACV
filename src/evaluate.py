...
def run_evaluation() -> None:
    ...
    results_reports_dir = root / "results" / "reports"
    ...
    for target_dir in (reports_dir, results_reports_dir):
        with (target_dir / "classification_report.txt").open("w", encoding="utf-8") as fh:
            fh.write(report_text)
        importance_df.to_csv(target_dir / "feature_importance.csv", index=False)
        if shap_df is not None:
            shap_df.to_csv(target_dir / "shap_summary.csv", index=False)
    ...
    md_content = "\n".join(
        [
            ...
            *[
            ...
            "",
            (
                "SHAP summary exported when the dependency is available."
                if shap_df is not None
    ...
    for target_dir in (reports_dir, results_reports_dir):
        with (target_dir / "evaluation_results.md").open("w", encoding="utf-8") as fh:
            fh.write(md_content)

    print(f"Evaluation complete. Artifacts written to {reports_dir} and {results_reports_dir}")
